...
class ICUUser(ICUMind):
    ...
    def revise(self, *perceptions):
        ...
        if len(self.priority) > 0:
            self.eye_to = self.task_positions[self.priority[0]][:2]
        else:
    ...
    def decide(self):
        ...
        if self.is_looking_at("fuel"):
            self.handle_fuel(actions)
        elif self.is_looking_at("system"):
            self.handle_system(actions)
        elif self.is_looking_at("track"):
            self.handle_tracking(actions)
        else: 
            pass 
        
        actions.append(self.move_eye(dx,dy))

        #print("FUEL:", self.is_looking_at("fuel"))
